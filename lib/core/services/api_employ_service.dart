...
class ApiService {
  ...
  Future<List<Employment>> fetchEmployments({int numOfRows = 100}) async {
    // 기본적으로 100개의 항목 요청
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (itemsData != null) {
          if (itemsData is List) {
            // itemsData가 리스트인 경우
            employments = itemsData.map((item) => Employment.fromJson(item)).toList();
          } else if (itemsData is Map<String, dynamic>) {
            // itemsData가 단일 객체인 경우
            employments = [Employment.fromJson(itemsData)];
          }
        }
